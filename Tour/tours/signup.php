...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($data['action'])) {
  ...
  if ($action === 'final_register') {
    ...
    // 👉 Store in session only (instead of inserting to database)
    $_SESSION['user'] = [
...
<body>
  ...
  ">
    <!-- Replace the circular div with GIF -->
    <div style="
      width: 64px;
      height: 64px;
      display: flex;
    ...
    ">
      <img src="../assets/img/illustration/check.gif" alt="Success" style="width: 64px; height: 64px; object-fit: contain;" />
    </div>
  ...
  <!-- Updated JavaScript function to show the success popup -->
  <script>
    ...
    // Update your verification success handler
      ...
      fetch("signup.php", {
        ...
        .then(data => {
          if (data.status === "success") {
            // Show the GIF success popup instead of regular popup
            showSuccessPopup();
            localStorage.removeItem("registeredUser");

            setTimeout(() => {
              // After successful signup, send stored tour or package info if available
              if (nextPageType === "custom") {
                const custom = localStorage.getItem("custom_tour_info");
                if (custom) {
                  fetch("save-custom-tour-after-signup.php", {
                    method: "POST",
                    headers: {
                      "Content-Type": "application/json"
                    },
                    ...
                    window.location.href = "custom-booking.php";
                  }).catch(() => {
                    window.location.href = "custom-booking.php";
                  });
                } else {
                ...
                const pack = localStorage.getItem("bookingData");
                if (pack) {
                  fetch("save-booking-after-signup.php", {
                    method: "POST",
                    headers: {
                      "Content-Type": "application/json"
                    },
                    ...
                    window.location.href = "package-booking.php";
                  }).catch(() => {
                    window.location.href = "package-booking.php";
                  });
                } else {
            ...
            }, 2500); // Increased timeout to let the GIF play
          } else {
            showPopup(data.message || "Incorrect code", "error");
          }
        })
        .catch(() => showPopup("Verification failed", "error"));
    });
  </script>
...
<script>
  ...
  document.addEventListener("DOMContentLoaded", function() {
              ...
              if (nextPageType === "custom") {
                ...
                if (custom) {
                  fetch("save-custom-tour-after-signup.php", {
                    ...
                    // localStorage.removeItem("custom_tour_info");
                    window.location.href = "custom-booking.php";
                  }).catch(() => {
                    // Even if error, proceed to next page
                    window.location.href = "custom-booking.php";
                  });
                } else {
                  window.location.href = "custom-booking.php";
    ...
    document.getElementById("resendCodeLink").addEventListener("click", function(e) {
      e.preventDefault();
      const user = JSON.parse(localStorage.getItem("registeredUser"));
      if (!user) return showPopup("No user data", "error");
      ...
      fetch("signup.php", {
        ...
        .then(res => res.json())
        .then(data => {
          if (data.status === "success") showPopup("New code sent", "success");
          else showPopup(data.message || "Resend failed", "error");
        }).catch(() => showPopup("Resend error", "error"));
    });
...
<script>
  window.addEventListener('DOMContentLoaded', () => {
    if (localStorage.getItem('bookingData')) {
      sessionStorage.setItem('bookingData', localStorage.getItem('bookingData'));
      console.log('Booking data copied from localStorage to sessionStorage');
    } else {
      console.log('No bookingData found in localStorage at signup');
    }
  });
</script>
